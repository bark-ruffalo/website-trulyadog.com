...
contract StakingVault is Ownable, ReentrancyGuard, Pausable {
    ...
    event Unlocked(address indexed user, uint256 indexed lockId, uint256 amount, uint256 poolId);
    ...
    function addPool(
        ...
        require(_lockPeriods.length == _rewardRates.length, "Mismatched lock periods and rates");
     ...
     */
    function updateRewardRates(uint256 poolId, uint256[] memory newRewardRates) external onlyOwner {
        require(poolId < pools.length, "Pool does not exist");
        Pool storage pool = pools[poolId];
        require(pool.lockPeriods.length == newRewardRates.length, "Mismatched lock periods and reward rates");
     ...
     */
    function stake(uint256 poolId, uint256 _amount, uint256 _lockPeriod) external whenNotPaused {
     ...
     */
    function lock(address user, uint256 poolId, uint256 amount, uint256 lockPeriod) internal {
     ...
     */
    function unlock(address user, uint256 lockId) internal returns (uint256, bool) {
        require(user != address(0), "Invalid user address");
     ...
     */
    function calculateRewards(address user, uint256 lockId) public view returns (uint256) {
        uint256 rewards = 0;
        uint256 currentTime = block.timestamp;
        LockInfo memory lockInfo = userLocks[user][lockId];

        // Fetch reward rate
        uint256 rewardRate = getRewardRate(lockInfo.poolId, lockInfo.lockPeriod);
        require(rewardRate > 0, "Invalid lock period.");

        if (lockInfo.isLocked) {
            uint256 stakingTime = currentTime -
                (lockInfo.lastClaimTime > 0 ? lockInfo.lastClaimTime : lockInfo.unlockTime - lockInfo.lockPeriod);
     ...
     */
    function getRewardRate(uint256 poolId, uint256 lockPeriod) internal view returns (uint256) {
        Pool storage pool = pools[poolId];
     ...
     * @notice Only callable by the contract owner.
     */
    function recoverTokens(IERC20 tokenAddress, address to, uint256 amount) external onlyOwner {
        ...
        for (uint256 i = 0; i < lockedUsers.length; i++) {
            for (uint256 j = 0; j < userLocks[lockedUsers[i]].length; j++) {
                ...
                if (lockInfo.isLocked) {
                    ...
                    emit Unlocked(lockedUsers[i], lockInfo.lockId, lockInfo.amount, lockInfo.poolId);
                }
            }
        }
    }

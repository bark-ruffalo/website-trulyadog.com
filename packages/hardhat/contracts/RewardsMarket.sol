...
contract RewardsMarket is Ownable, ReentrancyGuard, Pausable {
    ...
    function createCampaign(
        ...
        emit CampaignCreated(campaignId, minBurnAmount, endDate, tokenAddress, recipientAddress);
    }
     ...
     */
    function triggerReward(uint256 campaignId, uint256 burnAmount) external nonReentrant whenNotPaused {
        Campaign storage campaign = campaigns[campaignId];

        if (!campaign.isActive) revert CampaignNotActive();
        if (campaign.endDate != 0 && block.timestamp > campaign.endDate) revert CampaignExpired();
        if (campaign.maxRewards != 0 && campaign.rewardsIssued >= campaign.maxRewards) revert MaxRewardsReached();
        if (burnAmount < campaign.minBurnAmount) revert InsufficientBurnAmount();
        ...
        if (campaign.tokenAddress == address(0)) {
            ...
            require(campaign.recipientAddress != address(0), "Recipient not set for custom token");
            IERC20(campaign.tokenAddress).safeTransferFrom(msg.sender, campaign.recipientAddress, burnAmount);
        }

        // Execute external call if configured
        bool callSuccess = true;
        if (campaign.targetContract != address(0)) {
            (callSuccess, ) = campaign.targetContract.call(campaign.targetCalldata);
     ...
     */
    function getUserParticipationCount(address user, uint256 campaignId) external view returns (uint256) {
    ...
    function getActiveCampaigns(
        uint256 startIndex,
        uint256 endIndex
    ) external view returns (uint256[] memory campaignIds, Campaign[] memory details) {
        require(startIndex < endIndex, "Invalid range");
        require(endIndex <= allCampaignIds.length, "End index out of bounds");
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (
                ...
                (campaigns[campaignId].endDate == 0 || campaigns[campaignId].endDate > block.timestamp)
            ) {
                activeCount++;
            }
        }
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (campaign.isActive && (campaign.endDate == 0 || campaign.endDate > block.timestamp)) {
